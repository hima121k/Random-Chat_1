...
export default function Home() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 md:p-8 relative overflow-y-auto overflow-x-hidden bg-rc-bg text-rc-text">
      
      {/* Premium background decorative blur orbs */}
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        <div className="absolute -top-40 -left-40 w-[500px] h-[500px] bg-rc-accent/15 rounded-full blur-[120px] animate-pulse-slow" />
        <div className="absolute -bottom-40 -right-40 w-[500px] h-[500px] bg-indigo-700/10 rounded-full blur-[120px] animate-pulse-slow" style={{ animationDelay: '1.5s' }} />
        <div className="absolute top-1/3 left-1/3 w-[300px] h-[300px] bg-violet-900/10 rounded-full blur-[100px]" />
      </div>

      {/* Main container with responsive split layout on desktop */}
      <div className="w-full max-w-5xl z-10 flex flex-col lg:flex-row items-center lg:items-center justify-center gap-8 lg:gap-16 my-auto">
        
        {/* Left Side: Branding & Stats (always prominent but adjusts to left side on desktop) */}
        <div className="flex flex-col justify-center items-center lg:items-start text-center lg:text-left max-w-md w-full shrink-0">
          <div className="flex justify-center lg:justify-start mb-6">
            <div className="relative">
              <div className="absolute inset-0 bg-rc-accent rounded-3xl blur-xl opacity-60 animate-pulse-slow" />
              <div className="relative bg-gradient-to-br from-rc-accent to-indigo-600 p-5 rounded-3xl shadow-glow animate-float">
                <Zap size={48} className="text-white" strokeWidth={2.5} />
              </div>
            </div>
          </div>
          
          <h1 className="text-5xl lg:text-6xl font-extrabold mb-3 bg-gradient-to-r from-rc-accentGlow via-violet-300 to-indigo-300 bg-clip-text text-transparent tracking-tight">
            RandomChat
          </h1>
          <p className="text-rc-muted text-base lg:text-lg mb-6 leading-relaxed max-w-sm">
            Connect instantly with verified strangers worldwide. Private, end-to-end encrypted &amp; lightning fast.
          </p>

          {/* Premium Features List */}
          <div className="flex flex-col gap-3.5 w-full bg-rc-surface/40 border border-rc-border/40 p-5 rounded-2xl backdrop-blur-md mb-6 max-w-sm">
            <div className="flex items-center gap-3 text-sm text-rc-text">
              <div className="w-8 h-8 rounded-xl bg-rc-accent/10 border border-rc-accent/20 flex items-center justify-center shrink-0">
                <Lock size={15} className="text-rc-accentGlow" />
              </div>
              <div className="flex flex-col text-left">
                <span className="font-semibold text-xs">End-to-End Encrypted</span>
                <span className="text-[10px] text-rc-muted">Keys derived locally in your browser</span>
              </div>
            </div>
            <div className="flex items-center gap-3 text-sm text-rc-text">
              ...
              <div className="flex flex-col text-left">
                <span className="font-semibold text-xs">100% Anonymous</span>
                <span className="text-[10px] text-rc-muted">No real name or private info shared</span>
              </div>
            </div>
            <div className="flex items-center gap-3 text-sm text-rc-text">
              ...
              <div className="flex flex-col text-left">
                <span className="font-semibold text-xs">Instant Matchmaking</span>
                <span className="text-[10px] text-rc-muted">Connect with active users in seconds</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 w-full max-w-lg flex flex-col justify-center items-center">

          {/* ── BANNED SCREEN ── */}
          {isBanned ? (
            <div className="card p-8 w-full text-center space-y-4 border-red-500/30">
              ...
              {appealSent ? (
                ...
                <div className="text-left space-y-3">
                  ...
                  <button
                    ...
                    className="w-full py-2 bg-red-500/20 hover:bg-red-500/30 text-red-400 border border-red-500/40 rounded-xl text-sm font-semibold transition-colors disabled:opacity-50 cursor-pointer"
                  >
              ...
              <button onClick={handleSignOut} type="button"
                className="mt-2 px-6 py-2 bg-rc-surface hover:bg-rc-bg text-rc-muted border border-rc-border rounded-xl text-sm transition-colors cursor-pointer">
                Sign Out
              </button>
            </div>
            ...
            /* ── AUTH CARD ── */
            <div className="card p-7 w-full space-y-5">
              ...
              {authMode === 'login' && (
                <>
                  ...
                  <button onClick={handleGoogleSignIn} disabled={isLoading} type="button"
                    className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-50 text-gray-800 font-semibold py-3 rounded-xl shadow-lg transition-all disabled:opacity-60 text-sm cursor-pointer">
                  ...
                  <form onSubmit={handleEmailLogin} className="space-y-3 w-full">
                    ...
                    <button type="submit" disabled={isLoading} className="btn-primary w-full py-3 text-sm cursor-pointer">
                  ...
                  <p className="text-center text-sm text-rc-muted">
                    New here?{' '}
                    <button onClick={() => { setAuthMode('signup-email'); clearErrors() }} type="button"
                      className="text-rc-accentGlow hover:underline font-medium cursor-pointer">
                      Create account
                    </button>
                  </p>
                </>
              ...
              {authMode === 'signup-email' && (
                <>
                  ...
                  <form onSubmit={handleCreateAccount} className="space-y-3 w-full">
                    <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                  ...
                  <button onClick={() => { setAuthMode('login'); clearErrors() }} type="button"
                    className="w-full text-center text-sm text-rc-muted hover:text-rc-text underline cursor-pointer">
                    ← Back to Sign In
                  </button>
                </>
              )}
                  ...
                  <form onSubmit={handleSendOTP} className="space-y-3 w-full">
                    <div className="flex gap-2">
                      <select value={countryCode} onChange={e => setCountryCode(e.target.value)}
                        className="bg-rc-bg/80 border border-rc-border text-rc-text rounded-xl px-3 py-3 outline-none focus:border-rc-accent focus:ring-1 focus:ring-rc-accent/40 text-sm cursor-pointer">
              ...
              {authMode === 'signup-otp' && (
                <>
                  ...
                  <button onClick={() => { setAuthMode('signup-phone'); setOtp(''); clearErrors() }} type="button"
                    className="w-full text-center text-sm text-rc-muted hover:text-rc-text underline cursor-pointer">
                    ← Change phone number
                  </button>
                </>
              ...
              {error && (
                <div className="text-red-400 bg-red-400/10 border border-red-400/20 px-4 py-2.5 rounded-xl text-sm text-center w-full">
                  {error}
                </div>
              )}
            </div>
            ...
            /* ── CHAT SETUP CARD (Stunning spacious redesign!) ── */
            <div className="card p-6 md:p-7 w-full space-y-5">

              {/* Avatar Creation CTA */}
              <div className="bg-gradient-to-r from-rc-accent/10 to-indigo-500/10 border border-rc-accent/20 rounded-xl p-3.5 flex items-center justify-between shadow-glowSm">
                <div className="flex items-center gap-3">
                  {avatarUrl ? (
                    <img src={avatarUrl.replace('.glb', '.png')} alt="avatar" className="w-11 h-11 rounded-full ring-2 ring-rc-accent shadow-glowSm object-cover" />
                  ) : (
                    <div className="w-11 h-11 rounded-full bg-rc-surface border border-rc-border flex items-center justify-center">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-rc-muted mt-0.5">Stand out in the chat</p>
                  </div>
                </div>
                <button 
                  onClick={() => setShowAvatarCreator(true)}
                  type="button"
                  className="text-[10px] px-3.5 py-2 bg-rc-surface border border-rc-border rounded-lg text-rc-text hover:border-rc-accent/50 hover:bg-rc-accent/10 transition-all font-bold shrink-0 cursor-pointer"
                >
              ...
              <div className="flex gap-3">
                <div className="flex-1">
                  <label className="block text-[10px] font-bold text-rc-muted mb-1.5 uppercase tracking-wider">Nickname</label>
                  <input type="text" value={name} onChange={e => setName(e.target.value)}
                    placeholder="Your nickname" maxLength={20} className="input-field text-sm" />
                </div>
                <div className="w-24">
                  <label className="block text-[10px] font-bold text-rc-muted mb-1.5 uppercase tracking-wider">Age</label>
                  <input type="number" value={age} onChange={e => setAge(e.target.value)}
                    placeholder="18" min={13} max={99} className="input-field text-sm" />
                </div>
              </div>

              {/* Gender */}
              <div>
                <label className="block text-[10px] font-bold text-rc-muted mb-1.5 uppercase tracking-wider">I am a</label>
                <div className="flex gap-2.5">
                  {[
                    ...
                    <label key={val} className={`flex-1 text-center py-2.5 rounded-xl cursor-pointer border text-xs font-semibold transition-all ${
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-[10px] font-bold text-rc-muted mb-1.5 uppercase tracking-wider">Set your vibe</label>
                  <div className="flex gap-2 overflow-x-auto pb-1.5 hide-scrollbar">
                    {['Chill', 'Curious', 'Funny', 'Deep talk'].map((m) => (
                      <button
                        ...
                        type="button"
                        onClick={() => setMood(m)}
                        className={`px-3 py-1.5 rounded-full text-[11px] font-semibold whitespace-nowrap transition-all border cursor-pointer ${
                          mood === m
                ...
                <div className="flex flex-col gap-3">
                  <div>
                    <label className="block text-[10px] font-bold text-rc-muted mb-1.5 uppercase tracking-wider">Location (Optional)</label>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-[10px] font-bold text-rc-muted uppercase tracking-wider">Match with</label>
                  {!subscription.isPro && (
                    <div onClick={() => navigate('/pricing')} className="flex items-center gap-1 text-[9px] font-black text-amber-400 cursor-pointer hover:text-amber-300 transition-colors uppercase tracking-widest bg-amber-400/10 px-2.5 py-0.5 rounded-full">
                      <Star size={9} className="fill-amber-400" />
                      Pro
                    </div>
                  )}
                </div>
                <div className={`flex gap-2.5 ${!subscription.isPro ? 'opacity-60 grayscale' : ''}`}>
                  {[
                ...
                {!subscription.isPro && (
                  <p className="text-[9px] text-rc-muted mt-1.5 italic text-center leading-none">Upgrade to Pro to filter matches by gender</p>
                )}
              </div>

              <button onClick={handleStart} disabled={!name.trim()} type="button"
                className="btn-primary w-full py-3.5 text-sm font-bold tracking-wider flex items-center justify-center gap-2 cursor-pointer shadow-glow hover:shadow-none transition-all">
                <Zap size={16} strokeWidth={2.5} />
                Start Chatting
              </button>

              {error && (
                <div className="text-red-400 bg-red-400/10 border border-red-400/20 px-4 py-2.5 rounded-xl text-xs text-center w-full">
                  {error}
                </div>
              )}
            </div>
            ...
            /* ── MATCHING SPINNER ── */
            <div className="flex flex-col items-center space-y-5 w-full max-w-sm bg-rc-surface/40 border border-rc-border/40 p-8 rounded-3xl backdrop-blur-md text-center shadow-2xl animate-fade-in">
              <div className="relative">
              ...
              <div>
                <p className="text-rc-text font-bold text-lg">Finding your match…</p>
                <p className="text-rc-muted text-xs mt-1.5 leading-relaxed">This usually takes just a few seconds. Stay on this screen.</p>
              </div>
              <button onClick={handleCancel} type="button" className="text-rc-muted hover:text-rc-text underline text-xs transition-colors cursor-pointer">
                Cancel Matching
              </button>
            </div>
          )}
